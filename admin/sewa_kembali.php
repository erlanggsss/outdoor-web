...
<head>
	...
	<title>Rental Outdoor | Admin Kelola Sewa   </title>
								...
								<table id="zctb" class="display table table-striped table-bordered table-hover" cellspacing="0" width="100%">
									...
									<?php
										...
										$sqlsewa = "SELECT booking.*,produk.*,jenis_barang.*,users.* FROM booking,produk,jenis_barang,users WHERE booking.id_produk=produk.id_produk
													AND jenis_barang.id_barang=produk.id_barang AND users.email=booking.email AND status='Sudah Dibayar'
													ORDER BY booking.kode_booking DESC";
										$querysewa = mysqli_query($koneksidb,$sqlsewa);
										while ($result = mysqli_fetch_array($querysewa)) {
											$biayaproduk=$result['durasi']*$result['harga'];
											$total=$result['torguide']+$biayaproduk;
											$i++;
											?>
										<tr align="center">
											<td><?php echo $i;?></td>
